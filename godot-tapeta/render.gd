...
func createStarTexture() -> Texture2D:
	...
	var image: Image = Image.create(resolution, resolution, true, Image.FORMAT_RGBA8)
	...
	for y in range(resolution):
		for x in range(resolution):
			...
			var intensity: float = clampf(core + halo, 0.0, 1.0)
...
func createArms(rng: RandomNumberGenerator) -> void:
	for i in range(amount):
		...
		var angle: float = (TAU * float(k) / float(N)) + S * (r / size) * TAU + v
		...
		var scale_value: float = (maxR - r) * (rng.randf_range(min_scale, max_scale) / 10.0) + rng.randf_range(0.0, 0.2)

		addStar(i, position, scale_value, rng)
...
func createNoise(rng: RandomNumberGenerator) -> void:
	for i in range(noiseAmount):
		...
		addStar(i + amount, position, scale_value, rng)

func addStar(index: int, position: Vector3, scale_value: float, rng: RandomNumberGenerator) -> void:
	...
	var t: float = pow(rng.randf(), 3.0)
	var temperature: float = lerpf(2400.0, 30000.0, t) / 100.0
	var star_color: Color = temperatureToColor(temperature)

	multimesh.set_instance_color(index, star_color)

	pos.append(position)

	var x: float = position.x
	var z_pos: float = position.z
	var r2: float = x * x + z_pos * z_pos
	...
	var r: float = sqrt(r2)
	var orbit_x: float = -z_pos / r
	var orbit_z: float = x / r
	var speed: float = w * r

	vel.append(Vector2(orbit_x * speed, orbit_z * speed))

func updatePhysic(delta: float) -> void:
	var w2: float = w * w
	var use_mouse: bool = mouse_enabled and mousePositionValid

	var mouse_x: float = mouseWorldPosition.x
	var mouse_z: float = mouseWorldPosition.z
	var mouse_radius2: float = mouse_radius * mouse_radius
	var softening2: float = mouse_softening * mouse_softening
	var max_mouse_acc2: float = mouse_max_acceleration * mouse_max_acceleration
	...
	for i in range(pos.size()):
		...
		if use_mouse:
			...
			if d2 <= mouse_radius2:
				var denom: float = pow(d2 + softening2, 1.5)
				var factor: float = mouse_strength / denom
				var max_factor: float = mouse_max_acceleration / maxf(sqrt(d2), 0.0001)
...
func temperatureToColor(temperature: float) -> Color:
	...
	if temperature > 19.0 and temperature <= 66.0:
		blue = 138.5 * log(temperature - 10.0) - 305.04
	elif temperature > 66.0:
		blue = 255.0

	if temperature > 66.0:
		red = 329.6 * pow(temperature - 60.0, -0.13)
		green = 288.1 * pow(temperature - 60.0, -0.07)
	else:
...
func updateMouseWorldPosition() -> void:
	...
	var galaxy_plane: Plane = Plane(Vector3.UP, 0.0)
	var intersection: Variant = galaxy_plane.intersects_ray(ray_origin, ray_direction)

	if intersection == null:
		mousePositionValid = false
		return

	mouseWorldPosition = to_local(intersection as Vector3)
